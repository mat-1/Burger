...
class Mappings:
    ...
    def get_method_from_deobfuscated_name(self, obfuscated_class_name, method_name):
        for method_obfuscated_name, candidate_method_name in self.methods[obfuscated_class_name].items():
            print(candidate_method_name)
            if candidate_method_name == method_name:
                return method_obfuscated_name.split('(')[0]
        raise ValueError(f'Method {method_name} not found in class {obfuscated_class_name}')
